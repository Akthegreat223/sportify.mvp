  ...
  const handleQuickJoin = async (
    ...
    try {
      ...
      const newPlayerCount = isJoining
        ? matchDoc.players.length + 1
        : matchDoc.players.length - 1;
      const shouldConfirm =
        isJoining && newPlayerCount >= matchDoc.maxPlayers - 2;

      await updateDoc(doc(db, "matches", matchId), {
        players: isJoining ? arrayUnion(profile.uid) : arrayRemove(profile.uid),
        status: shouldConfirm
          ? "confirmed"
          : matchDoc.status || "seeking_players",
      });

      if (isJoining) {
        if (shouldConfirm || matchDoc.status === "confirmed") {
          alert(
            `✅ Match confirmed! See you at ${formatDate(matchDoc.dateTime)} 🎉`,
          );
        } else {
          alert(
      ...
      console.log(
        `[EVENT]: Match ${isJoining ? "Joined" : "Left"} (Quick Join)`,
      );
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="space-y-8 pb-24">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-black italic uppercase tracking-tighter text-black dark:text-white">
            Games
          </h1>
            ...
            >
              ...
              <div className="mb-8">
                ...
                <p className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">
                  Gujrat Regional Office
                </p>
              </div>

              <form onSubmit={handleCreate} className="space-y-6">
                <div>
                  <label className="text-[10px] font-black uppercase text-neutral-500 mb-2 block">
                    Match Title
                  </label>
                  <input
                    required
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <optgroup label="Tomorrow">
                          <option value="tomorrow-09">
                            Tomorrow - 09:00 AM
                          </option>
                          <option value="tomorrow-17">
                            Tomorrow - 05:00 PM
                          </option>
                        </optgroup>
      ...
      <div className="space-y-6">
        {loading ? (
          [1, 2].map((i) => (
            <div
              key={i}
              className="h-32 bg-neutral-100 dark:bg-neutral-900 rounded-3xl animate-pulse"
            />
          ))
        ) : matches.length > 0 ? (
          matches.map((match) => {
            ...
            const isJoined = profile?.uid
              ? match.players.includes(profile.uid)
              : false;
            ...
            return (
              <div
                key={match.id}
                onClick={() => {
                  const matchExists = matches.find((m) => m.id === match.id);
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="space-y-3">
                      <div className="text-[10px] font-black text-primary uppercase tracking-[0.2em] italic">
                        {match.sport}
                      </div>
                    ...
                    <div className="bg-white/5 px-4 py-3 rounded-[1.5rem] border border-white/5 text-center min-w-[84px] flex flex-col items-center justify-center">
                      ...
                      <div className="text-[8px] font-bold text-neutral-500 uppercase tracking-tighter">
                        PLAYERS JOINED
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 pt-6 border-t border-white/5">
                    <div className="flex items-center gap-3 text-neutral-400">
                      ...
                      <span className="text-xs font-bold uppercase truncate tracking-wide">
                        {match.location}
                      </span>
                    </div>
                    <div className="flex items-center gap-3 text-neutral-400">
          ...
          <div className="py-20 text-center bg-neutral-950 rounded-[2.5rem] border border-white/10 shadow-2xl">
            ...
            <button
              onClick={() => setIsCreating(true)}
              className="mt-4 text-primary font-bold text-[10px] uppercase underline"
            >
              Host the first one
            </button>
          </div>
