...
import { Trophy, Users, MapPin, Calendar, Clock, ChevronLeft, Share2, Zap } from 'lucide-react';
...
export default function MatchDetails() {
  ...
  const handleJoinLeave = async () => {
    ...
    const newPlayerCount = isJoining ? match.players.length + 1 : match.players.length - 1;
    ...
    try {
      ...
      if (isJoining) {
        if (shouldConfirm || match.status === 'confirmed') {
          alert(`✅ Match confirmed! See you at ${formatDate(match.dateTime)} 🎉`);
        } else {
          alert("✅ You're in! Invite more players to confirm this match faster.");
        }
      }
      console.log(`[EVENT]: Match ${isJoining ? 'Joined' : 'Left'} (Details Page)`);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  if (!match) return (
    <div className="h-screen flex flex-col items-center justify-center p-8 text-center space-y-6 bg-white dark:bg-black transition-colors">
      <h2 className="text-4xl font-black uppercase italic tracking-tighter text-black dark:text-white">This match is still forming</h2>
      <p className="text-neutral-500 dark:text-neutral-400 max-w-xs uppercase font-bold text-[10px] tracking-widest">Join from the home screen or explore other games</p>
      <Link to="/">
        <Button className="rounded-full px-8 h-14 uppercase font-black italic">Return to Home</Button>
      </Link>
    </div>
  );
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="p-3 bg-neutral-100 dark:bg-neutral-900 rounded-full border border-neutral-200 dark:border-white/5 hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-colors text-black dark:text-white">
          <ChevronLeft className="w-6 h-6" />
        </button>
        <div>
          <h1 className="text-2xl font-black italic uppercase tracking-tighter leading-none text-black dark:text-white">Match Details</h1>
      ...
      <section className="bg-neutral-950 dark:bg-neutral-900/50 border border-white/10 rounded-[3rem] p-10 space-y-10 relative overflow-hidden shadow-2xl">
        ...
        <div className="space-y-4 relative z-10">
          <p className="text-[10px] font-black uppercase text-primary italic tracking-widest">{match.sport}</p>
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <h2 className="text-5xl font-black italic uppercase tracking-tighter leading-[1] text-white">{match.title || "Evening Tape Ball Match"}</h2>
          ...
          <div className="flex flex-wrap gap-6 pt-4">
            <div className="flex items-center gap-2 text-white">
               ...
               <span className="text-sm font-bold uppercase tracking-wide">{match.location}</span>
            </div>
            <div className="flex items-center gap-2 text-white">
               <Calendar className="w-5 h-5 text-primary" />
               <span className="text-sm font-bold uppercase tracking-wide">{formatDate(match.dateTime)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 pt-10 border-t border-white/5 relative z-10">
          <div>
            <p className="text-[10px] font-black uppercase text-neutral-500 tracking-widest mb-2">Team Size</p>
            <div className="text-3xl font-black italic text-white">{match.maxPlayers} <span className="text-xs text-neutral-500 uppercase">Players</span></div>
          </div>
          <div>
            <p className="text-[10px] font-black uppercase text-neutral-500 tracking-widest mb-2">Spots Open</p>
            <div className="text-3xl font-black italic text-white">{match.maxPlayers - match.players.length} <span className="text-xs text-neutral-500 uppercase">Left</span></div>
          </div>
        </div>
      ...
      <section className="px-4">
        <h3 className="text-[10px] font-black uppercase text-neutral-500 tracking-widest mb-6">Confirmed Players ({match.players.length})</h3>
        <div className="flex flex-wrap gap-4">
           {match.players.map((pid, index) => (
              <div key={`${pid}-${index}`} className="flex flex-col items-center gap-2">
                 <div className="w-16 h-16 rounded-full bg-neutral-900 border border-white/10 overflow-hidden shadow-lg">
                    <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${pid === 'system' ? `${pid}_${index}` : pid}&topType=shortHair,turban,shaved,hat,shortHair,shortHair,shortHair`} alt="P" />
                 </div>
           ...
           {match.players.length === 0 && <p className="text-neutral-500 text-xs font-bold uppercase italic">No one has joined yet. Be the first!</p>}
        </div>
      </section>
    </div>
  );
